...
def categorize(report_nm: str) -> str:
    """공시명 → 7 카테고리 (정기공시·주요사항·주주총회·배당·자본거래·수시공시·기타).

    순서: 정기 → 주주총회 → 배당 → 자본거래 → 주요사항 → 수시공시 → 기타.
    자본거래(증권 발행)를 주요사항보다 먼저 매칭해 "유상증자결정" 같은 케이스가
    명확히 자본거래로 분류되도록 함.
    """
    # 정기공시 — 사업/분기/반기 보고서 + 대기업 집단/지배구조/ESG 정기 보고서
    if any(k in report_nm for k in [
        "사업보고서", "분기보고서", "반기보고서",
        "기업집단현황", "지배구조보고서", "지속가능경영",
    ]):
        return "정기공시"
    ...
    # 자본거래 — 증자/감자/합병/분할 + 증권 발행/소각/타법인 출자
    if any(k in report_nm for k in [
        "증자", "감자", "합병", "분할",
        "투자설명서", "일괄신고", "증권발행", "증권신고서",
        "주식소각", "타법인주식", "출자증권취득",
    ]):
        return "자본거래"

    # 주요사항 — 주요사항보고서 + 지분 변동/계약/임원 관련
    if any(k in report_nm for k in [
        "주요사항보고서",
        "특정증권등소유", "대량보유", "최대주주", "주식담보",
        "단일판매", "공급계약",
        "투자판단", "주요경영사항",
        "채무보증", "특수관계인",
        "임원ㆍ주요주주", "사외이사", "임원의 선임", "임원의 해임", "임원ㆍ주요주주",
    ]):
        return "주요사항"

    # 수시공시 — 공정공시·수시공시·풍문해명·IR 행사
    if any(k in report_nm for k in [
        "공정공시", "수시공시",
        "풍문", "해명", "기업설명회",
    ]):
